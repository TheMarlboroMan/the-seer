...
/*	Uint8 alpha=acc_alpha();
	...
	//Implementación de Bresenham, fusilada.
	...
	for(int x=(int)x1; x<maxX; x++)
	{
		if(steep)
		{
			if(this->metodo_pixeles) pixel=this->metodo_pixeles(x, y, x1, y1, x2, y2, acc_parametro_metodo_pixeles());				
			Utilidades_graficas_SDL::SDL_PutPixel(superficie, p_caja, y, x, pixel, alpha);
		}
		else
		{
			if(this->metodo_pixeles) pixel=this->metodo_pixeles(x, y, x1, y1, x2, y2, acc_parametro_metodo_pixeles());				
			Utilidades_graficas_SDL::SDL_PutPixel(superficie, p_caja, x, y, pixel, alpha);
		}
